...
class ZS:
    ...
    def __post_init__(self):
        ...
        self.zg = self._calculate_zg()
        self.zd = self._calculate_zd()
    ...
    def __repr__(self):
        return (
            ...
            f"gg={self.gg}, zz={self.zz})"
        )

    def _calculate_zg(self) -> float:
        """计算中枢上限: 除去入中枢的一笔，后三笔才确定中枢的上下边"""
        if len(self.bis) < 4:  # 至少需要4笔才能形成有效中枢
            return 0.0
            
        # 中枢上限是后三笔的高点中的最小值
        # 跳过入中枢的第一笔，从第二笔开始取三笔
        return min([max(x.high, x.fx_b.high) for x in self.bis[1:4]])

    def _calculate_zd(self) -> float:
        """计算中枢下限: 除去入中枢的一笔，后三笔才确定中枢的上下边"""
        if len(self.bis) < 4:  # 至少需要4笔才能形成有效中枢
            return 0.0
            
        # 中枢下限是后三笔的低点中的最大值
        # 跳过入中枢的第一笔，从第二笔开始取三笔
        return max([min(x.low, x.fx_b.low) for x in self.bis[1:4]])
