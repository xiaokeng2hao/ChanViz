...
# logger.add("czsc_analyze.log", rotation="10 MB", level="DEBUG") # 删除或注释掉这行
...
def check_bi(bars: List[NewBar], **kwargs):
    ...
    logger.debug(f"check_bi: 输入 K 线数量: {len(bars)}, 识别分型数量: {len(fxs)}, 要求最小笔长度: {min_bi_len}")
    if len(bars) > 0:
        logger.debug(f"check_bi: 输入 K 线时间范围: {bars[0].dt} to {bars[-1].dt}")

    if len(fxs) < 2:
        logger.debug("check_bi: 分型数量 < 2，无法成笔")
        return None, bars

    fx_a = fxs[0]
    
    logger.debug(f"check_bi: 起始分型 fx_a: dt={fx_a.dt}, mark={fx_a.mark}, fx={fx_a.fx}")

    if fx_a.mark == Mark.D:
        direction = Direction.Up
        fxs_b = [x for x in fxs if x.mark == Mark.G and x.dt > fx_a.dt and x.fx > fx_a.fx]
        if not fxs_b:
            logger.debug("check_bi: 向上笔，未找到合适的顶分型 fx_b")
            return None, bars
        fx_b = max(fxs_b, key=lambda fx: fx.high)
        logger.debug(f"check_bi: 向上笔，找到候选 fx_b 数量: {len(fxs_b)}, 最终选择 fx_b: dt={fx_b.dt}, mark={fx_b.mark}, fx={fx_b.fx}, high={fx_b.high}")

    elif fx_a.mark == Mark.G:
        direction = Direction.Down
        fxs_b = [x for x in fxs if x.mark == Mark.D and x.dt > fx_a.dt and x.fx < fx_a.fx]
        if not fxs_b:
            logger.debug("check_bi: 向下笔，未找到合适的底分型 fx_b")
            return None, bars
        fx_b = min(fxs_b, key=lambda fx: fx.low)
        logger.debug(f"check_bi: 向下笔，找到候选 fx_b 数量: {len(fxs_b)}, 最终选择 fx_b: dt={fx_b.dt}, mark={fx_b.mark}, fx={fx_b.fx}, low={fx_b.low}")
        
    else:
        # 这个分支理论上不应该执行，因为 fx_a 的 mark 只能是 D 或 G
        logger.error(f"check_bi: fx_a 的 mark 异常: {fx_a.mark}")
        raise ValueError

    # 确保 fx_b 是存在的 (虽然前面的逻辑已经判断过，这里再加一层保险)
    if fx_b is None:
        logger.warning("check_bi: 逻辑错误，fx_b 不应为 None")
        return None, bars

    # 截取 fx_a 和 fx_b 之间的 K 线 (bars_a 用于判断长度和包含关系)
    bars_a = [x for x in bars if fx_a.elements[0].dt <= x.dt <= fx_b.elements[2].dt]
    # 剩余未处理的 K 线 (bars_b)
    bars_b = [x for x in bars if x.dt >= fx_b.elements[0].dt]
    
    bars_a_len = len(bars_a)
    if bars_a:
        logger.debug(f"check_bi: 候选笔 K 线范围 (bars_a): {bars_a[0].dt} to {bars_a[-1].dt}, 数量: {bars_a_len}")
    else:
        logger.debug("check_bi: 候选笔 K 线范围 (bars_a) 为空")

    # 判断 fx_a 和 fx_b 价格区间是否存在包含关系
    # 注意：这里的 high/low 是分型本身的 high/low，不是 fx 值
    ab_include = (fx_a.high >= fx_b.high and fx_a.low <= fx_b.low) or \
                 (fx_a.high <= fx_b.high and fx_a.low >= fx_b.low)
    logger.debug(f"check_bi: fx_a 和 fx_b 价格区间是否存在包含关系 (ab_include): {ab_include}")
    logger.debug(f"check_bi: fx_a (H:{fx_a.high}, L:{fx_a.low}), fx_b (H:{fx_b.high}, L:{fx_b.low})")

    # 成笔的条件：1）顶底分型之间价格区间没有包含关系；2）笔的无包含K线长度大于等于min_bi_len
    condition1_no_include = not ab_include
    condition2_length_ok = bars_a_len >= min_bi_len
    logger.debug(f"check_bi: 成笔条件检查 - 价格无包含 (condition1): {condition1_no_include}, 长度足够 (condition2): {condition2_length_ok} (实际 {bars_a_len} >= 要求 {min_bi_len})")
    
    if condition1_no_include and condition2_length_ok:
        ...
        logger.info(f"check_bi: *** 成功形成笔! *** 方向: {direction.value}, K线数量: {bars_a_len}, 起点: {fx_a.dt} ({fx_a.fx}), 终点: {fx_b.dt} ({fx_b.fx})")
        return bi, bars_b
    else:
        logger.debug(f"check_bi: 未满足成笔条件，返回 None")
        return None, bars
